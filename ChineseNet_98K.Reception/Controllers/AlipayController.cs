namespace ChineseNet_98K.Reception.Controllers
{
    ...
    public class AlipayController : Controller
    {
        ...
        private IConsumptions_BLL iConsumptions_BLL;
        private static int Balance { get; set; }
        private static int UserId { get; set; }

        public AlipayController(IAlipayService alipayService, IWallets_BLL _iWallets_BLL, IConsumptions_BLL _iConsumptions_BLL)
        {
        ...
        /// <returns></returns>
        [HttpPost]
        public void PayRequest(string tradeno, string subject, string totalAmout, string itemBody, int id)
        {
            // 组装业务参数model
            ...
            Balance = int.Parse(totalAmout) * 100;
            UserId = id;
            AlipayTradePagePayRequest request = new AlipayTradePagePayRequest();
            // 设置同步回调地址
            request.SetReturnUrl($"http://{Request.Host}/Alipay/Callback");
            // 设置异步通知接收地址
        ...
        public IActionResult Callback()
        {
			...
			*/
            ...
            if (sArray.Count != 0)
            {
               ...
               var m= iWallets_BLL.GetUserid(UserId);
                m.Balance = m.Balance+ Balance;

                Consumptions t = new Consumptions
                {
                ...
                var aa = iConsumptions_BLL.Add(t);
                var result = iWallets_BLL.Update(m);
                if (result > 0)
                {
                    Response.Redirect("/Home/Wallets");
                    ViewData["PayResult"] = "同步验证成功";
                }
                //}
                //else
                //{
